...
Includes:
- ModelArchitecture: Enum of supported SD models
- Sampler: Enum of supported samplers
- Discretization / Guider / Thresholder: Enums for sampler configs
- SamplingParams: Default sampling hyperparameters
- SamplingSpec: Model-specific configuration
- SamplingPipeline: Wrapper class for text-to-image, image-to-image, and refinement
- Helpers to build sampler, guider, and discretization configurations
"""
...
from bert.inference.helpers import Img2ImgDiscretizationWrapper, do_img2img, do_sample
from bert.modules.diffusionmodules.sampling import (
    DPMPP2MSampler,
...
from bert.util import load_model_from_config
...
class SamplingPipeline:
    """Stable Diffusion Sampling Wrapper for text2img, img2img, and refiner pipelines."""
...
def get_guider_config(params: SamplingParams):
    if params.guider == Guider.IDENTITY:
        return {"target": "bert.modules.diffusionmodules.guiders.IdentityGuider"}
    elif params.guider == Guider.VANILLA:
        scale = params.scale
        dyn_thresh_config = {
            "target": "bert.modules.diffusionmodules.sampling_utils.NoDynamicThresholding"
        } if params.thresholder == Thresholder.NONE else NotImplemented
        return {
            "target": "bert.modules.diffusionmodules.guiders.VanillaCFG",
...
def get_discretization_config(params: SamplingParams):
    if params.discretization == Discretization.LEGACY_DDPM:
        return {"target": "bert.modules.diffusionmodules.discretizer.LegacyDDPMDiscretization"}
    elif params.discretization == Discretization.EDM:
        return {
            "target": "bert.modules.diffusionmodules.discretizer.EDMDiscretization",
