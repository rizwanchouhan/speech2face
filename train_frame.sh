#!/usr/bin/env bash
# ==============================================================
# Keyframe Training Script
# ==============================================================
#
# This script launches training for the audio-conditioned
# keyframe generation model using PyTorch Lightning.
#
# Usage:
#   bash train_keyframes.sh <filelist> [num_workers] [batch_size] [num_gpus]
#
# Example:
#   bash train_keyframes.sh filelists/train.txt 8 2 4
#
# ==============================================================
...
if [ -z "$FILELIST_PATH" ]; then
  ...
  echo "Usage: bash train_keyframes.sh <filelist> [num_workers] [batch_size] [num_gpus]"
  exit 1
fi
...
# (change these if your storage changes)
# --------------------------------------------------------------
VIDEO_DIR="videos"
AUDIO_DIR="audios"
...
# --------------------------------------------------------------
CONFIG_PATH="configs/example_training/keyframes/keyframes_base.yaml"
BASE_LR="1.e-5"
...
python main.py \
  ...
  data.params.train.datapipeline.audio_emb_type=${AUDIO_EMB_TYPE} \
  ...
  model.params.loss_fn_config.params.weight_pixel=${PIXEL_WEIGHT} \
  "model.params.loss_fn_config.params.what_pixel_losses=${PIXEL_LOSSES}" \
